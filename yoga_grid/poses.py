...
def _worse_leg_extended(view: PoseView) -> float:
    ...
    **已知脆弱之处（2026-08 实测）。** 侧面机位下，远侧那半边身体每一帧都被
    近侧挡住，MediaPipe 给出的是**猜测**而不是测量：某支 38 分钟的视频里
    右膝置信度中位数 0.315，80% 的帧低于 0.5。于是取 min 有一半概率读到猜出来
    的那个膝 —— 实测该视频 49% 的帧如此，山式那一簇是 52%（置信度中位 0.480、
    最低 0.181）。

    它在那批数据上仍然给出了正确结果，但那是运气：MediaPipe 对被遮挡的肢体
    倾向于**照着可见那一侧复制**，所以猜出来的膝读到 175~180「伸直」，恰好
    和真实情况一致。反过来的情形会失效 —— 一条腿真的屈着、而且正好是被挡住
    的那条，猜测会说它是直的，min 就读不到那个屈膝。

    根本原因在分层：``normalize()`` 只保留 (33, 2)，把置信度那一列丢掉了，
    所以规则层**无法知道哪些关键点是真的**。要修就得让 ``PoseView`` 拿到
    置信度，对称体式优先采信可信的那一侧，而不是把测量值和猜测值一起平均。
    那是一次跨 landmarks / poses / extract / report 的改动，尚未做。
    """
    return min(
        view.ang("s_hip", "s_knee", "s_ankle"),
        view.ang("o_hip", "o_knee", "o_ankle"),
...
def _knee_drop(view: PoseView) -> float:
    ...
    俯卧支撑类体式和**跪姿**之间的分水岭。直臂斜板的膝在肩踝连线上，身体与
    地面约 25°，所以膝只比髋低 0.27~0.45；四足跪姿/桌面式的膝直接压在髋
    正下方的地面上，实测 0.87~0.98。

    真实视频里 121 帧「直臂斜板」有 46 帧是跪姿（膝角 67~88°、膝落地），
    直臂斜板六项里只有「身体成一直线」正确地判 0 分（实测 126~146°），
...
def _lower_knee_drop(view: PoseView) -> float:
    ...
    算出来是 0.57，恰好钻过直臂斜板「膝盖不能落地」那道门槛的上界 0.60，
    于是被直臂斜板认走 0.93。取 max 则立刻读到 0.92。

    「膝盖不能落地」问的是**有没有**膝落地，所以要看更低的那个膝。

    **下界同样是必须的，这一条后补。** 三个支撑体式最初都写成 ``-2.0``，等于
    只问「膝有没有太低」，从不问「膝有没有太高」。代价：用户 30 分钟视频里
    一段 98 秒的仰卧开背练习（屈膝踩地、双膝高过髋 0.24~0.42）就从下界的
    缺口钻了进去 —— 40 帧里 30 帧被侧板式认走、10 帧被前臂平板认走，还各占
    了九宫格一格。

    实测把两边分得很干净：撑地的体式最低是四柱支撑式 +0.16、直臂斜板 +0.23，
    而那 98 秒最高只到 -0.24。下界取 0.00、slack 0.20，两边各留 0.16 / 0.24。
    """
...
def _wrist_below_elbow(view: PoseView) -> float:
    ...
    （标准四柱支撑式 0.43、直臂斜板 0.50、上犬式 0.42），小臂贴地时肘腕
    几乎同高（实测前臂平板 0.05~0.15）。
...
def _feet_below_hips(view: PoseView) -> float:
    ...
    都在 0.5 附近）和双腿伸直程度太接近，光靠朝向门槛分不开。同理，一个直臂斜板
...
TEMPLATES: tuple[Template, ...] = (
    ...
    Template(
        ...
        checks=(
            ...
            # 双臂伸直撑地全都一样），头的朝向是唯一可靠的区别：直臂斜板低头。
            Check("头部朝下（俯卧）", lambda v: v.dy("nose", "shoulder_mid"), -0.30, 0.30, 0.40, 2.5, ""),
            Check("双臂伸直", _arms_extended, 176, 12, 30, 1.0),
        ),
        min_score=0.62,
        # 「身体成一直线」是直臂斜板的评分项，塌腰的直臂斜板仍然是直臂斜板，所以它
        # 不能当门槛。区别「差的直臂斜板」和「根本不是直臂斜板」的是膝有没有落地。
        gates=(Gate("膝要在髋下方，且不落地", _lower_knee_drop, 0.00, 0.60, 0.20),),
    ),
    Template(
        key="warrior2",
        zh="战士二式",
    ...
    Template(
        ...
        checks=(
            ...
            # 没有这一项，一个直臂斜板能在本模板上拿到 0.8 以上 —— 躯干水平、
            # 双腿伸直、抬起腿水平三项它全满足。
    ...
    Template(
        ...
        gates=(
            ...
            # 9.5，判 0 分也只扣 16%，直臂斜板照旧能拿 0.83。上界 168° 留着
            # 12° 余量给标准骨架（156.1°），而直臂斜板最小 175.5°。
    ...
    Template(
        ...
        spine_up=(-0.45, 0.45),
        checks=(
            # 屈肘 90° 是与直臂斜板唯一的区别（直臂斜板双臂伸直约 176°），
            # 所以这一项权重最高。
        ...
        min_score=0.64,
        # 与直臂斜板同一条门槛。给直臂斜板补上之后，那 46 帧四足跪姿立刻改投
        # 四柱支撑式（0.80）—— 跪姿屈着的肘正好满足「屈肘约 90°」。
        # 补门槛把误判推给邻居，就得顺着推下去补完。
        gates=(
            Gate("膝要在髋下方，且不落地", _lower_knee_drop, 0.00, 0.60, 0.20),
            # 四柱支撑式是**手**撑地、肘向后夹在肋侧，所以腕比肘低半条小臂
            # （标准骨架 0.43）。前臂平板是小臂整条贴地、肘腕同高（实测 0.09）。
            # 「屈肘约 90°」两者都满足，所以光靠肘角分不开 —— 实测 v1 里
            # 20:40~21:34 那 18 帧前臂平板，有 14 帧被这个模板认走 0.90~1.00。
    ...
    Template(
        ...
        # 全部满足。
        #
        # 直臂斜板和四柱支撑式**不加**这条同名门槛，是刻意的：那两个体式塌腰
        ...
        gates=(
            ...
            # 「上手朝天、只有一只手撑地」是侧板式区别于直臂斜板的定义 —— 上面
            ...
            # 上面那条「身体必须成一直线」量的是肩-髋-**踝**，跳过了膝。仰卧
            # 屈膝踩地时，把膝折上去、脚踩在身后，肩-髋-踝依然读 174° ——
            # 门槛一分不扣。实测代价：用户一段 98 秒的仰卧开背练习，30 帧被
            # 这里认走 0.70（门槛 0.64），还占了九宫格一格。
            #
            # 和直臂斜板/四柱/前臂平板那三条同源，数值也取一样的：撑地的体式
            # 实测最低 +0.16，那 98 秒最高 -0.24。
            Gate("膝要在髋下方", _lower_knee_drop, 0.00, 3.0, 0.20),
        ),
    ),
    Template(
        key="reverse_plank",
        ...
        spine_up=(-0.30, 0.55),
        checks=(
            # 直臂斜板和反板式在 2D 剪影上接近镜像，最可靠的区别是头的朝向：
            # 直臂斜板低头（鼻低于肩），反板式仰头（鼻高于肩）。权重给足。
            ...
            # 与直臂斜板同理必须双手撑地 —— 侧板式一手朝天，只看主侧会被它蒙混。
    ...
    Template(
        ...
        # 四足跪姿的膝压在髋正下方 0.80。给直臂斜板和四柱式补上膝落地门槛后，
    ...
    # 整簇 16 帧、四足跪姿→直臂斜板 46 帧、展背式→站立前屈式 0.91）。给那些
    ...
    Template(
        ...
        checks=(
            # 小臂整条贴地是前臂平板的定义，肘腕几乎同高 —— 实测 0.05~0.15。
            # 手撑地的那些体式是 0.42~0.50。
        ...
        gates=(
            ...
            # 样本离得很开（前臂平板 -0.06~0.00，四柱 0.29~0.30），但 0.16~0.24
    ...
    Template(
        key="back_opener",
        zh="开背",
        en="Supine Back Opener",
        symmetric=True,
        # 躯干接近水平、肩略高于髋（上背离地）。实测 +0.05~+0.20。
        spine_up=(-0.25, 0.45),
        checks=(
            # 两膝都高过髋 —— 这一项同时是下面的门槛，见那里的注释。
            Check("双膝高过髋（仰卧屈膝）", _higher_knee_drop, -0.54, 0.22, 0.30, 2.5, ""),
            Check("屈膝约 66°", _legs_extended, 66, 18, 30, 2.0),
            Check("双脚踩实地面", lambda v: v.dy("s_knee", "s_ankle"), 0.49, 0.18, 0.25, 2.0, ""),
            Check("双脚并拢", _ankle_spread, 0.03, 0.22, 0.30, 1.0, ""),
            Check("上背离地、头高过肩", lambda v: v.dy("shoulder_mid", "nose"), -0.21, 0.18, 0.25, 2.0, ""),
            Check("躯干接近水平", lambda v: v.horiz("shoulder_mid", "hip_mid"), 6, 16, 28, 1.5),
            # 远侧手臂在侧面机位下每一帧都被挡住（实测置信度中位 0.12），
            # 所以这一项权重最低 —— 它会被 report.md 标成「看着达标其实没测准」，
            # 那正是想要的：说出来，但别让它左右判定。
            Check("双臂沿地面伸过头顶", lambda v: v.dy("shoulder_mid", "s_wrist"), 0.14, 0.20, 0.28, 1.0, ""),
        ),
        min_score=0.62,
        gates=(
            # **两膝都高过髋**，这是开背区别于全部其它模板的那一条。
            #
            # 其余 25 具标准骨架里最低的是战士三式 +0.00、四柱支撑式 +0.08 ——
            # 没有一个已建模的体式膝高过髋；实测这 40 帧是 -0.60~-0.47。
            # 上界取 -0.20、slack 0.15：战士三式的 +0.00 高出上界 0.20、
            # 超过 slack，整个归零；开背最高的 -0.47 还留着 0.27 余量。
            #
            # 用 _higher_knee_drop（两膝里离地更远的那个）而不是中点：中点会被
            # 「一膝抬起、一膝落地」抵消成中间值，这个坑在 _lower_knee_drop
            # 的注释里已经记了三次。这里要的是**两膝都**抬起来。
            Gate("两膝必须都高过髋", _higher_knee_drop, -3.0, -0.20, 0.15),
        ),
        # 容差**只按一次练习（那 98 秒 40 帧）定的，还偏宽**：这 40 帧现在
        # 每一帧都是 1.00，`tools/calibrate.py` 会如实把大部分检查项标成
        # 「饱和」。这是刻意的 —— 按单支视频收紧，等于把某一天的状态写死成
        # 标准（README「按实测分布核对容差」一节）。等有两三支视频再收。
    ),
    Template(
        key="chair",
        zh="幻椅式",
    ...
    Template(
        ...
        checks=(
            # 膝压在髋正下方的地面上 —— 实测 0.87~0.98。直臂斜板是 0.27~0.45。
