...
* 直臂斜板在战士三式模板上拿到 0.8（战士三式缺站姿约束）；
* 双臂上举的战士一式被判成战士二式（战士二式只查两腕连线水平，
  而上举时两腕同样等高）；
* 侧板式被直臂斜板认走（直臂斜板只查主侧那只手，侧板式撑地的手蒙混过关）；
* 反板式被直臂斜板认走（2D 剪影近乎镜像，缺头部朝向这一对称判据）。
* **完全平直的身体被上犬式认走**（真实视频，2026-08）。上犬式的「躯干后弯」
  原先取 150±22/slack 38 —— 衰减到 0 的位置在 210°，于是一个 175.5° 的
  平直身体还能拿 0.91 分，8 帧直臂斜板被认成上犬式 0.98，九宫格里同一个姿势
...
#: 曾被直臂斜板认走 0.62~0.75，补上门槛后依次落到四柱支撑式 0.80、桥式 0.69、
...
#: 半神猴式 1.00 → 直臂斜板 0.93 → 侧角伸展式 0.72 → 上犬式 0.72 → 侧板式 0.67 →
#: 未识别。每一轮补的都是那个模板真正的定义性特征，不是为它量身定做的补丁：
#:
#: * 半神猴式：伸直的腿必须伸向**前**方（它伸向后方，投影 -1.55 对 +1.50）；
#: * 直臂斜板：膝盖不能落地 —— 但 ``_knee_drop`` 取两膝**中点**，一膝落地
...
def test_forearm_plank_and_chaturanga_split_on_where_the_arm_rests():
    """前臂平板 vs 四柱支撑式：唯一的区别是撑地的是手还是整条小臂。

    两者的肘角都在 90~100°、身体都成一直线、都是俯卧 —— 「屈肘约 90°」这一项
    分不开它们。分水岭是腕比肘低多少：手撑地时低半条小臂（标准四柱骨架 0.43），
    小臂贴地时肘腕同高（实测前臂平板 -0.06~0.00）。

    真实代价：缺前臂平板模板时，某支视频里 20:40~21:34 那 18 帧前臂平板有 14 帧
